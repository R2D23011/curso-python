...
def generate_bar_chart(name, labels, values):
  fig, ax = plt.subplots()
  #son dos valores que nos da la librería, fig es como la figura y ax se refire a las 
  # coordenadas donde  vamos a empezar a graficar
  ax.bar(labels, values)
  #aquí le estás indicando que quieres generar una gráfica de barras (bar), y le envías
  #labels y values para que sepa que tiene que crear el gráfico con esos valores
...
def generate_pie_chart(name, labels, values):
 ...
 Aquí le estamos indicando que queremos que nos muestre una gráfica de torta, 
  ...
  Ahora le indicamos los labels 
  pero también tenemos que indicarle como van a ser los labels 
  """
  ax.axis('equal')
  plt.savefig(f'./imgs/{name}.png')
  plt.close()
